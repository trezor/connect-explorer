...
const PREFIX: string = 'tron_getaddress';
...
export function onGetAddress(tx: string): any {
    return async function (dispatch, getState) {
        ...
        if (bundled.length > 1) {
            ...
            response = await TrezorConnect.tronGetAddress({bundle});
        } else {
            response = await TrezorConnect.tronGetAddress(getState().common.params);
        }
